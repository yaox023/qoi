...
const QOI_TAG_MASK = 0xc0;
const QOI_OP_INDEX_TAG = 0x00;
const QOI_OP_INDEX_MASK = 0x3f;
const QOI_OP_DIFF_TAG = 0x40;
const QOI_OP_DIFF_DR_MASK = 0x30;
const QOI_OP_DIFF_DG_MASK = 0x0c;
const QOI_OP_DIFF_DB_MASK = 0x03;
const QOI_OP_DIFF_BIAS = 2;
const QOI_OP_LUMA_TAG = 0x80;
const QOI_OP_LUMA_DG_MASK = 0x3f;
const QOI_OP_LUMA_G_BIAS = 32;
const QOI_OP_LUMA_DRDG_MASK = 0xf0;
const QOI_OP_LUMA_DBDG_MASK = 0x0f;
const QOI_OP_LUMA_RB_BIAS = 8;
const QOI_OP_RUN_TAG = 0xc0;
const QOI_OP_RUN_MASK = 0x3f;
const QOI_OP_RUN_BIAS = -1;
const QOI_OP_RGB_TAG = 0xfe;
const QOI_OP_RGBA_TAG = 0xff;
const INIT_PIXEL = [0, 0, 0, 255];
const isEqual = (cur, pre) => {
    return (cur[0] === pre[0] &&
        cur[1] === pre[1] &&
        cur[2] === pre[2] &&
        cur[3] === pre[3]);
...
const hash = (pixel) => (pixel[0] * 3 + pixel[1] * 5 + pixel[2] * 7 + pixel[3] * 11) % 64;
export const encode = (qoiFile) => {
    ...
    let diff = [0, 0, 0, 0];
    const wraparound = (n) => n & 0b10000000 ? (n - 256) % 256 : (n + 256) % 256;
    let drdg = 0;
    let dbdg = 0;
    const calculateDiff = () => {
    ...
    const writeOpLuma = () => {
        ...
        writeByte(((drdg + QOI_OP_LUMA_RB_BIAS) << 4) | (dbdg + QOI_OP_LUMA_RB_BIAS));
    };
    const writeOpRGBA = () => {
...
export const decode = (qoiBytes) => {
    ...
    const isStreamEnd = () => {
        ...
        for (let i = 0; i < QOI_END_MARKER.length; i++) {
            if (qoiBytes[readIndex + i] !== QOI_END_MARKER[i])
                return false;
        }
        return true;
    };
            ...
            default: {
                switch (byte & QOI_TAG_MASK) {
                    ...
                    case QOI_OP_LUMA_TAG: {
                        ...
                        const drdg = ((nextByte & QOI_OP_LUMA_DRDG_MASK) >> 4) - QOI_OP_LUMA_RB_BIAS;
                        const dbdg = ((nextByte & QOI_OP_LUMA_DBDG_MASK) >> 0) - QOI_OP_LUMA_RB_BIAS;
                        prevPixel[0] += drdg + dg;
                        prevPixel[1] += dg;
                        prevPixel[2] += dbdg + dg;
                        writePixel();
